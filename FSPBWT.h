...
//     double outPanelFilteringTime = 0;
//     double outPanelIdentificationTime = 0;
//     int inPanelAlternativeNum = 0;
//     int inPanelMatchNum = 0;
//     int outPanelAlternativeNum = 0;
//     int outPanelMatchNum = 0;
//     vector<string> IDs;
//     vector<int> physLocs;
//     vector<vector<Syllable> > X; // MN bits
//     vector<vector<uint32_t> > fuzzyX; // FMN/B bits
//     vector<vector<unsigned short> > array; // 32MN/B bits
//     vector<vector<int> > divergence; // 32MN/B bits
//     //vector<vector<vector<int>>> u; //	32MN2^F/B bits
//     unsigned short *u;
//
...
/*
 int catchFuzzy(int index, int k) const {
 //int fuzzy=0;
 int unit_index = k * F / 32;
 int offset = k * F % 32;
 uint32_t currentElement = fuzzyX[index][unit_index];
 if (F + offset > 32)
 {
 uint32_t nextElement = fuzzyX[index][unit_index + 1];
 // 第一个元素中可用的 bit 位数
 size_t availableBits = 32 - offset;
 // 从第一个元素中获取的数据
 uint32_t dataFromFirstElement = currentElement
 & ((1 << availableBits) - 1);

 // 从第二个元素中获取的数据
 uint32_t dataFromSecondElement = nextElement
 >> (32 - (F + offset - 32));

 // 合并两个元素中的数据
 return ((dataFromFirstElement << (F + offset - 32))
 | dataFromSecondElement);
 }
 else
 {
 // 根据偏移量和数据占位数 F，提取对应数据并返回
 uint32_t mask = (1 << F) - 1; // 用于提取数据的掩码
 uint32_t extractedData = (currentElement >> (32 - offset - F)) & mask;
...
int FSPBWT<Syllable>::load(const char *save_file) {
    ...
    // Load vectors with dimension information
    ...
    //loadVectorVectorVectorWithDimension(in, u);
...
//
// template<class Syllable>
// int FSPBWT_v1<Syllable>::readVCF(string panel_file) {
//     clock_t start, end;
//     start = clock();
//     ifstream in(panel_file);
//     if (in.fail())
...
//     X.resize(M, vector<Syllable>(n)); //600M
//     X.shrink_to_fit();
//     array.resize(n + 1, vector<unsigned short>(M));
//     array.shrink_to_fit();
//     divergence.resize(n + 1, vector<int>(M, 0));
//     divergence.shrink_to_fit();
//     iota(array[0].begin(), array[0].end(), 0);
...
//     //u.resize(n, vector<vector<int>>(M, vector<int>(T)));
//     // 为三维数组分配内存
//     //u=allocate_3d_array( n,  M,  T);
//     // unsigned short *u_data = new unsigned short[n * M * T]; // 连续内存块
//     // u = new unsigned short **[n]; // 分配指针数组
//     // for (int i = 0; i < n; i++) {
//     //     u[i] = new unsigned short *[M];
//     //     for (int j = 0; j < M; j++) {
//     //         u[i][j] = u_data + (i * M * T) + (j * T); // 指向连续内存块
//     //     }
//     // }
//     //
//     // // 分配 u_data 为多个连续内存块
//     // std::vector<unsigned short*> u_data_blocks(n);
//     // for (int i = 0; i < n; i++) {
//     //     u_data_blocks[i] = new unsigned short[M * T];
//     // }
//     //
//     // // 分配指针数组
//     // u = new unsigned short**[n];
//     // for (int i = 0; i < n; i++) {
//     //     u[i] = new unsigned short*[M];
//     //     for (int j = 0; j < M; j++) {
//     //         u[i][j] = u_data_blocks[i] + (j * T); // 指向每个块的起始地址
//     //     }
//     // }
//     unsigned short *u = new unsigned short[(long)n * M * T];  // 单块分配
//     // u = new unsigned short**[n];
//     // for (int i = 0; i < n; i++) {
//     //     u[i] = new unsigned short*[M];
//     //     for (int j = 0; j < M; j++) {
//     //         u[i][j] = u_data + (i * M * T) + (j * T);  // 指针算术
//     //     }
//     // }
//     //return 0;  2146688
//     vector<Syllable> X_(M); //X in syllable k
...
// }
// template<class Syllable>
// int FSPBWT_v1<Syllable>::readQueryVCF(string query_file) {
//     clock_t start, end;
//     start = clock();
//
//     ifstream in(query_file);
...
//
// template<class Syllable>
// int FSPBWT_v1<Syllable>::makeFuzzyPanelEvenly() {
//     //get fuzzyX
//     clock_t start, end;
//     start = clock();
//     if (FF * n % 32 != 0) {
...
//     //u.resize(n, vector<vector<int>>(M, vector<int>(T)));
//
//     vector<unsigned short> a[T]; // make array[][]  his size for u[][]
//     vector<int> d[T]; //  make divergence
//     vector<int> p(T); //  make divergence
//     for (int _ = 0; _ < T; _++) {
//         a[_].resize(M);
...
//         }
//         int unit_index = k / (32 / FF);
//         int offset = 32 - FF - k % (32 / FF) * FF; //get fuzzy from fuzzyX
//
//         for (int i = 0; i < M; i++) //each hapolotype
//         {
//             //update u[][][]
//             for (int _ = 0; _ < T; _++) {
//                 u[k * (M * T) + i * T + _]=a[_].size();
//                 //u[k][i][_] = a[_].size();
//                 if (divergence[k][i] > p[_]) {
//                     p[_] = divergence[k][i];
//                 }
...
//             }
//
//             a[fuzzy].push_back(index);
//             d[fuzzy].push_back(p[fuzzy]);
//             p[fuzzy] = 0;
//         } //end j in  M
//
//         //update array divergence u[][][]
...
//
// template<class Syllable>
// int FSPBWT_v1<Syllable>::makeFuzzyPanelGlobally() // fuzzy way :overall
// {
//     //get fuzzyX
//     clock_t start, end;
//     start = clock();
...
//
// template<class Syllable>
// int FSPBWT_v1<Syllable>::inPanelLongMatchQuery(int L, string inPanelOutput_file) {
//     clock_t start, end;
//     start = clock();
//     if (L < minSiteL) {
//         return 1;
...
//
// // template<class Syllable>
// // int FSPBWT_v1<Syllable>::outPanelLongMatchQuery(int L, string outPanelOutput_file, bool even) {
// //     clock_t start, end;
// //     start = clock();
// //
// //     if (FF * n % 32 != 0) {
...
//
// template<class Syllable>
// void FSPBWT_v1<Syllable>::inPanelRefine(int L, int s_idx, int e_idx, int index_a, int index_b,
//                                         ofstream &out) {
//     int start = 0, end = 0;
//     if (s_idx == -1) {
//         start = 0;
...
//
// template<class Syllable>
// void FSPBWT_v1<Syllable>::inPanelIdentification(int L, int s_idx, int e_idx, int index_a,
//                                                 int index_b, ofstream &out) {
//     clock_t start, end;
//     start = clock();
//     int l = (L - 2 * (B - 1)) / B;
...
//
// template<class Syllable>
// void FSPBWT_v1<Syllable>::outPanelIdentification(int L, int s_idx, int e_idx, int index_a,
//                                                  int index_b, ofstream &out) {
//     clock_t start, end;
//     start = clock();
//     int l = (L - 2 * (B - 1)) / B;
...
//
// template<class Syllable>
// void FSPBWT_v1<Syllable>::outPanelRefine(int L, int s_idx, int e_idx, int index_a, int index_b,
//                                          ofstream &out) {
//     int start = 0, end = 0;
//     if (s_idx == -1) {
//         start = 0;
...
//  }*/
// template<class Syllable>
// void FSPBWT_v1<Syllable>::outputInformationToFile(const std::string &fileName, string mode) {
//     std::ofstream outputFile(fileName);
//
//     if (outputFile.is_open()) {
//         if (mode == "in") {
...
//
// template<class Syllable>
// int FSPBWT_v1<Syllable>::save(string save_file) {
//     ofstream out(save_file, ios::binary);
//     if (!out.is_open())
//         return 1;
//
...
//
// template<class Syllable>
// int FSPBWT_v1<Syllable>::load(const char *save_file) {
//     ifstream in(save_file, ios::binary);
//     if (!in.is_open())
//         return 1;
//
